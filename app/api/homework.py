...
async def update_homework(
    ...
    if image and image.filename:
        ...
        if homework.image_path:
            old_filename = homework.image_path.replace("/uploads/", "")
        ...
        try:
            with open(destination, "wb") as buffer:
                shutil.copyfileobj(image.file, buffer)
            homework.image_path = f"/uploads/{unique_filename}"
        except Exception as e:
    ...
    try:
        ...
        await db.rollback()
        raise HTTPException(status_code=500, detail=f"Vazifani tahrirlashda xatolik: {str(e)}")
