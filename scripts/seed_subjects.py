...
from app.models import Subject, Topic, TopicStatus, User, UserRole

async def seed():
    async with AsyncSessionLocal() as session:
        # Find employee user (either id 1 or the first employee in db)
        res_emp = await session.execute(select(User).where(User.role.in_([UserRole.employee, UserRole.superadmin])))
